...
import {
  LOCAL_OPERATION_PRESENCE_EVENT,
  isLocalOperationPresenceFresh,
  readLocalOperationPresence,
  type LocalOperationPresenceRecord,
} from '@/features/operation/local-operation-presence';
import type { Condominium } from '@/types/condominium';

type MasterSection = 'overview' | 'create' | 'licenses' | 'modules' | 'monitoring';
...
async function createClientWithModules(form: BootstrapClientForm, modules: Record<string, boolean>) {
  const payload = {
    ...
    licensePlan: form.licensePlan.trim() || undefined,
    licenseStatus: form.licenseStatus,
    licenseExpiresAt: form.licenseExpiresAt || undefined,
    enabledModules: enabledModuleKeys(modules),
    moduleSettings: modules,
    slimMode: !modules.people || !modules.units,
  };
...
async function updateClientLicense(id: string, payload: { licensePlan: string; licenseStatus: LicenseStatus; licenseExpiresAt?: string | null }) {
...
export default function MasterPage() {
  ...
  useEffect(() => {
    const validSections = new Set<MasterSection>(['overview', 'create', 'licenses', 'modules', 'monitoring']);
  ...
  const backendEnabledCount = useMemo(
    () => (moduleCatalog.length ? getCatalogBackedCount(modules) : getBackendSupportedModuleCount(modules)),
    [moduleCatalog.length, modules]
  );
  const backendEditingEnabledCount = useMemo(
    () => (moduleCatalog.length ? getCatalogBackedCount(editingModules) : getBackendSupportedModuleCount(editingModules)),
    [editingModules, moduleCatalog.length]
  );
  const filteredClientList = useMemo(() => {
    ...
    return monitoredClients.filter((item) => {
      const matchesSearch =
        !search ||
        [item.name, item.document, item.responsibleName, item.city, item.state]
          .filter(Boolean)
      ...
      if (clientSort === 'offline') {
        const healthDelta = Number(isOperationOffline(b)) - Number(isOperationOffline(a));
  ...
  function hydrateSelectedClient(condominium: Condominium) {
    ...
    setLicenseForm({
      ...
      licensePlan: condominium.licensePlan || 'Profissional',
  ...
  async function loadCondominiums() {
    ...
    try {
      const [data, summaryResponse, moduleCatalogResponse, devicesResponse] = await Promise.all([
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const { condominium, fullContractPersisted } = await createClientWithModules(form, modules);
      ...
      setMessage(
        fullContractPersisted
          ? `Cliente "${mergedCondominium.name}" criado com ${enabledCount} módulo(s) habilitado(s). ${backendEnabledCount} já persistidos pela API v4.4.`
          : `Cliente "${mergedCondominium.name}" e admin criados. A API ainda precisa persistir endereço, documento, licença e módulos.`
      );
    } catch (submitError) {
  ...
  function handleExportClientsCsv() {
    ...
    const lines = filteredClientList.map((item) => {
      ...
      return [
        ...
        getBackendSupportedModuleCount(normalizedModules),
        getApiCoverage(normalizedModules),
      ]
        .map(escapeCsv)
        .join(';');
    });
  ...
  async function handleSaveModules() {
    ...
    try {
      ...
      setMessage(`Cliente "${merged.name}" atualizado com ${editingEnabledCount} módulo(s) habilitado(s). ${backendEditingEnabledCount} já persistidos pela API v4.4.`);
    } catch (saveError) {
  ...
  async function handleSaveLicense() {
    ...
    try {
      const profilePayload = {
        ...
        clientKind: licenseForm.clientKind,
        document: licenseForm.document.trim() || null,
        address: licenseForm.address.trim() || null,
        city: licenseForm.city.trim() || null,
        state: licenseForm.state.trim() || null,
        zipCode: licenseForm.zipCode.trim() || null,
        responsibleName: licenseForm.responsibleName.trim() || null,
        responsibleEmail: licenseForm.responsibleEmail.trim() || null,
        responsiblePhone: licenseForm.responsiblePhone.trim() || null,
      };
      const licensePayload = {
        licensePlan: licenseForm.licensePlan.trim(),
        ...
        <CardContent className="space-y-2">
          <div className="space-y-3">
            <input
              ...
              placeholder="Buscar cliente, documento ou responsável"
            ...
            <div className="grid grid-cols-4 gap-2">
              {[
                ...
                { key: 'no_heartbeat', label: 'Sem heartbeat' },
              ].map((option) => (
                <button
                  key={option.key}
                  type="button"
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { key: 'offline', label: 'Priorizar offline' },
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-400">{kind === 'RESIDENCE' ? 'Residência' : 'Condomínio'} | {compactId(condominium.id)}</p>
                    </div>
                  ...
                  <div className="mt-3">
                    <div className="flex items-center justify-between gap-3 text-[11px] text-slate-500">
                      <span>Cobertura da API</span>
                      <span>{getApiCoverage(normalizeModulesFromCondominium(condominium))}%</span>
                    </div>
                    <div className="mt-2 h-2 rounded-full bg-white/10">
                      <div
                        className="h-full rounded-full bg-emerald-400"
                        style={{ width: `${getApiCoverage(normalizeModulesFromCondominium(condominium))}%` }}
                      />
                    </div>
                  </div>
                  <p className="mt-3 text-[11px] text-slate-500">
                    {getBackendSupportedModuleCount(normalizeModulesFromCondominium(condominium))} módulo(s) persistidos na API
                  </p>
                  <p className="mt-3 text-[11px] text-slate-500">
  ...
  const moduleGrid = (target: 'create' | 'edit', values: Record<string, boolean>) => (
    <div className="space-y-3">
      ...
      <div className={`rounded-2xl border p-4 text-xs text-white ${brandClasses.softAccentPanel}`}>
        API v4.4 persiste hoje os módulos: Usuários, Unidades, Pessoas, Câmeras, Alertas, Acessos, Encomendas,
        Visitas previstas, Veículos, Facial, Mensagens, Operação e Monitoramento. Os demais continuam visíveis no produto como planejamento e UX,
        mas ainda dependem de expansão do catálogo no backend para persistirem por cliente.
      </div>
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {module.backendSupported ? (
                    <span className="rounded-full border border-emerald-400/20 bg-emerald-400/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-emerald-100">
                      API v4.4
                    </span>
                  ) : (
                    <span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-amber-100">
                      Planejado
                    </span>
                  )}
                </div>
    ...
    return (
      <div className="space-y-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <StatTile title="Moradores/Pessoas" value={summary.peopleCount} description="Depende do contador global da API" icon={Users} tone="emerald" />
          <StatTile title="Câmeras" value={summary.cameraCount} description="Câmeras cadastradas nos clientes" icon={Camera} />
          <StatTile title="Portarias offline" value={summary.offlineOperations} description="Computadores sem heartbeat recente" icon={AlertTriangle} tone={summary.offlineOperations ? 'rose' : 'emerald'} />
          <StatTile title="Persistidos na API" value={summary.backendSupportedModulesTotal} description="Módulos já cobertos pela v4.4" icon={ShieldCheck} tone="emerald" />
          <StatTile title="Planejamento visual" value={summary.plannedModulesTotal} description="Módulos ainda fora do catálogo backend" icon={Puzzle} tone={summary.plannedModulesTotal ? 'amber' : 'cyan'} />
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <Card className="border-white/10 bg-white/5 text-white">
            ...
            <CardContent className="space-y-4">
              {[
                { label: 'Licenças ativas ou em teste', value: summary.activeLicenses, total: Math.max(summary.clients, 1), tone: 'bg-emerald-400' },
                { label: 'Modulos habilitados', value: summary.enabledModulesTotal, total: Math.max(summary.clients * platformModules.length, 1), tone: 'bg-cyan-400' },
                { label: 'Módulos persistidos pela API', value: summary.backendSupportedModulesTotal, total: Math.max(summary.clients * platformModules.length, 1), tone: 'bg-emerald-400' },
                { label: 'Módulos ainda planejados', value: summary.plannedModulesTotal, total: Math.max(summary.clients * platformModules.length, 1), tone: 'bg-amber-400' },
                { label: 'Clientes com portaria offline', value: summary.offlineOperations, total: Math.max(summary.clients, 1), tone: 'bg-rose-400' },
              ].map((item) => (
                <div key={item.label} className="rounded-2xl border border-white/10 bg-slate-950/45 p-4">
                  ...
                  <div className="mt-3 h-2 rounded-full bg-white/10">
                    <div className={`h-full rounded-full ${item.tone}`} style={{ width: `${Math.min(100, (item.value / item.total) * 100)}%` }} />
                  </div>
                </div>
              ))}
            </CardContent>
            ...
            <CardContent className="space-y-3">
              <div className="rounded-2xl border border-white/10 bg-slate-950/45 p-4">
                ...
                <p className="mt-2 text-sm text-slate-300">
                  Use esta tela só para acompanhar volume, licenças, cobertura da API e saúde geral das portarias.
                </p>
                <p className="mt-2 text-xs text-slate-500">
                  Para editar cliente, revisar licença, módulos ou dispositivos, use as seções específicas acima.
                </p>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
                  <p className="text-xs uppercase tracking-[0.16em] text-amber-100">Sem heartbeat</p>
                  <p className="mt-2 text-center text-3xl font-semibold tabular-nums text-white">{clientsWithoutHeartbeat}</p>
                  <p className="mt-2 text-xs text-amber-100/80">Clientes aguardando primeiro sinal operacional.</p>
                </div>
                <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                  ...
                  <p className="mt-2 text-xs text-emerald-100/80">Clientes com sinal operacional recente.</p>
                </div>
              </div>
            </CardContent>
          </Card>
  ...
  function renderCreate() {
    ...
    return (
      <form onSubmit={handleSubmit} className="grid gap-4 xl:grid-cols-[1fr_0.95fr]">
        <Card className="border-white/10 bg-white/5 text-white">
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-950/45 p-4">
              <p className="text-sm font-medium text-white">Licença inicial</p>
              <div className="mt-3 grid gap-3 md:grid-cols-3">
                <Field label="Plano" value={form.licensePlan} onChange={(value) => setField('licensePlan', value)} placeholder="Profissional" />
                <SelectField label="Status" value={form.licenseStatus} onChange={(value) => setField('licenseStatus', value)} options={licenseStatuses} />
                <Field label="Validade" value={form.licenseExpiresAt} onChange={(value) => setField('licenseExpiresAt', value)} type="date" />
              </div>
            </div>
        ...
        <Card className="border-white/10 bg-white/5 text-white">
          <CardHeader>
            <div className="flex items-center justify-between gap-3">
              <CardTitle className="flex items-center gap-2 text-lg"><Puzzle className={`h-5 w-5 ${brandClasses.accentTextSoft}`} />Modulos iniciais</CardTitle>
              <span className="text-xs text-slate-400">
                {enabledCount}/{platformModules.length} total | {backendEnabledCount} persistidos pela API
              </span>
            </div>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex flex-wrap gap-2">
              <button type="button" onClick={() => enableAll('create')} className="rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-xs text-white hover:bg-white/15">Habilitar todos</button>
              <button type="button" onClick={() => enableCoreOnly('create')} className="rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2 text-xs text-slate-200 hover:bg-white/10">Só essenciais</button>
            </div>
            {moduleGrid('create', modules)}
          </CardContent>
        </Card>
      </form>
    );
  ...
  function renderLicenses() {
    const licenseDocumentLabel = licenseForm.clientKind === 'RESIDENCE' ? 'CPF do responsável' : 'CNPJ do condomínio';
    ...
    return (
      <section className="grid gap-4 xl:grid-cols-[360px_1fr]">
        ...
        <Card className="border-white/10 bg-white/5 text-white">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg"><KeyRound className={`h-5 w-5 ${brandClasses.accentTextSoft}`} />Cadastro e licença</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {selectedCondominium ? (
              <>
                <div className="grid gap-3 sm:grid-cols-2">
                  <button
                    type="button"
                    onClick={() => setLicenseForm((prev) => ({ ...prev, clientKind: 'CONDOMINIUM' }))}
                    className={`rounded-2xl border p-4 text-left transition ${licenseForm.clientKind === 'CONDOMINIUM' ? brandClasses.activeTab : 'border-white/10 bg-slate-950/45 hover:bg-white/10'}`}
                  >
                    <Building2 className={`h-5 w-5 ${brandClasses.accentTextSoft}`} />
                    <p className="mt-3 font-medium text-white">Condomínio</p>
                    <p className="mt-1 text-xs text-slate-400">Contrato condominial com CNPJ.</p>
                  </button>
                  <button
                    type="button"
                    onClick={() => setLicenseForm((prev) => ({ ...prev, clientKind: 'RESIDENCE' }))}
                    className={`rounded-2xl border p-4 text-left transition ${licenseForm.clientKind === 'RESIDENCE' ? brandClasses.activeTab : 'border-white/10 bg-slate-950/45 hover:bg-white/10'}`}
                  >
                    <Home className="h-5 w-5 text-emerald-200" />
                    <p className="mt-3 font-medium text-white">Residência</p>
                    <p className="mt-1 text-xs text-slate-400">Contrato residencial com CPF.</p>
                  </button>
                </div>

                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Nome do cliente" value={licenseForm.name} onChange={(value) => setLicenseForm((prev) => ({ ...prev, name: value }))} />
                  <Field label={licenseDocumentLabel} value={licenseForm.document} onChange={(value) => setLicenseForm((prev) => ({ ...prev, document: value }))} />
                  <Field label="Endereço" value={licenseForm.address} onChange={(value) => setLicenseForm((prev) => ({ ...prev, address: value }))} className="md:col-span-2" />
                  <Field label="Cidade" value={licenseForm.city} onChange={(value) => setLicenseForm((prev) => ({ ...prev, city: value }))} />
                  <Field label="UF" value={licenseForm.state} onChange={(value) => setLicenseForm((prev) => ({ ...prev, state: value.toUpperCase().slice(0, 2) }))} />
                  <Field label="CEP" value={licenseForm.zipCode} onChange={(value) => setLicenseForm((prev) => ({ ...prev, zipCode: value }))} />
                  <Field label="Responsável" value={licenseForm.responsibleName} onChange={(value) => setLicenseForm((prev) => ({ ...prev, responsibleName: value }))} />
                  <Field label="E-mail do responsável" value={licenseForm.responsibleEmail} onChange={(value) => setLicenseForm((prev) => ({ ...prev, responsibleEmail: value }))} type="email" />
                  <Field label="Telefone" value={licenseForm.responsiblePhone} onChange={(value) => setLicenseForm((prev) => ({ ...prev, responsiblePhone: value }))} />
                </div>

                <div className="grid gap-3 md:grid-cols-3">
                  <Field label="Plano" value={licenseForm.licensePlan} onChange={(value) => setLicenseForm((prev) => ({ ...prev, licensePlan: value }))} />
                  ...
                  <Field label="Validade" value={licenseForm.licenseExpiresAt} onChange={(value) => setLicenseForm((prev) => ({ ...prev, licenseExpiresAt: value }))} type="date" />
                </div>

                <div className="grid gap-3 md:grid-cols-3">
                  <MiniInfo title="Tipo" value={licenseForm.clientKind === 'RESIDENCE' ? 'Residência' : 'Condomínio'} />
                  <MiniInfo title="Documento" value={licenseForm.document || 'Não informado'} />
                  <MiniInfo title="Vencimento" value={formatDate(licenseForm.licenseExpiresAt)} />
                </div>
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-400">
                  {selectedCondominium
                    ? `${editingEnabledCount} módulo(s) habilitado(s), ${backendEditingEnabledCount} já suportados pela API v4.4.`
                    : 'Selecione um cliente para editar.'}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => enableAll('edit')} disabled={!selectedCondominium} className="rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-xs text-white hover:bg-white/15 disabled:opacity-50">Habilitar todos</button>
                <button type="button" onClick={() => enableCoreOnly('edit')} disabled={!selectedCondominium} className="rounded-lg border border-white/10 bg-slate-950/50 px-3 py-2 text-xs text-slate-200 hover:bg-white/10 disabled:opacity-50">Só essenciais</button>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className={`rounded-2xl border p-4 text-sm text-white ${brandClasses.softAccentPanel}`}>
              API preferencial: {'`PATCH /api/v1/master/clients/{client_id}/modules`'}. Fallback legado: {'`PATCH /api/v1/condominiums/{id}`'}.
            </div>
            <button type="button" onClick={() => void handleSaveModules()} disabled={!selectedCondominium || savingEdit} className={`inline-flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-60 ${brandClasses.solidAccent}`}>
    ...
    return (
      <div className="space-y-4">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <StatTile title="Portarias offline" value={summary.offlineOperations} description="Sem heartbeat recente" icon={AlertTriangle} tone={summary.offlineOperations ? 'rose' : 'emerald'} />
          <StatTile title="Licenças ativas" value={summary.activeLicenses} description="Ativas ou em teste" icon={Activity} tone="emerald" />
          <StatTile title="Sem heartbeat" value={clientsWithoutHeartbeat} description="Cliente sem sinal inicial" icon={RefreshCw} tone={clientsWithoutHeartbeat ? 'amber' : 'cyan'} />
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
            ...
            <p className="mt-2 text-xs text-emerald-100/80">Com heartbeat recente e sem alerta operacional.</p>
          </div>
          <div className="rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4">
            ...
            <p className="mt-2 text-xs text-amber-100/80">Cliente ainda aguardando o primeiro heartbeat.</p>
          </div>
          <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4">
            ...
            <p className="mt-2 text-xs text-rose-100/80">Offline ou sem heartbeat.</p>
          </div>
        </div>
          ...
          <Card className="border-white/10 bg-white/5 text-white">
            ...
            <CardContent className="space-y-3">
                ...
                return (
                  <div key={client.id} className="grid gap-3 rounded-2xl border border-white/10 bg-slate-950/45 p-4 md:grid-cols-[1fr_auto_auto] md:items-center">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-400">Último sinal: {client.operationLastSeenAt ? `${formatDate(client.operationLastSeenAt)} ${new Date(client.operationLastSeenAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : 'aguardando API'}</p>
                    </div>
                    <span className={`rounded-full px-3 py-1 text-xs ${noHeartbeat ? 'bg-amber-500/15 text-amber-100' : offline ? 'bg-rose-500/15 text-rose-100' : 'bg-emerald-500/15 text-emerald-100'}`}>{noHeartbeat ? 'Sem heartbeat' : offline ? 'Desconectado' : 'Conectado'}</span>
              ...
              <div className="rounded-2xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm text-amber-100">
                Para saber quando um computador desconectar em tempo real, o backend precisa registrar heartbeat por dispositivo e avisar via WebSocket/SSE.
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-white/10 bg-white/5 text-white">
            ...
            <CardContent className="space-y-3">
              <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                ...
                <p className="mt-2 text-xs text-emerald-100/80">Clientes com heartbeat recente.</p>
              </div>
              <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-4">
                  ...
                  {priorityMonitoringClients.map((client) => (
                    ...
                    >
                      ...
                      <span className={`rounded-full border px-2 py-1 text-[10px] uppercase ${!client.operationLastSeenAt ? 'border-amber-500/20 bg-amber-500/10 text-amber-100' : 'border-rose-500/20 bg-rose-500/10 text-rose-100'}`}>
                        {!client.operationLastSeenAt ? 'Sem heartbeat' : 'Offline'}
                      </span>
                    </button>
                  ))}
  ...
  return (
    <div className="space-y-6 text-white">
      <section className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur">
        <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Master</p>
        <div className="mt-2 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <h1 className="text-3xl font-semibold">Gestão global de clientes</h1>
            <p className="mt-3 max-w-3xl text-sm text-slate-400">
              Cadastre condomínios e residências, gerencie licenças, módulos contratados e acompanhe a saúde operacional das portarias.
            </p>
          </div>
          <div className="flex flex-wrap gap-3">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/10 px-4 py-3 text-sm text-white hover:bg-white/15 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Download className="h-4 w-4" />
              Exportar clientes
            </button>
            <button type="button" onClick={() => void loadCondominiums()} className="inline-flex items-center justify-center gap-2 rounded-lg border border-white/10 bg-white/10 px-4 py-3 text-sm text-white hover:bg-white/15">
              <RefreshCw className={`h-4 w-4 ${loadingCondominiums ? 'animate-spin' : ''}`} />
              Atualizar dados
            </button>
          </div>
      ...
      {message ? <div className={`rounded-2xl border px-4 py-3 text-sm text-white ${brandClasses.softAccentPanel}`}>{message}</div> : null}
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`min-w-[180px] flex-1 rounded-2xl border p-4 text-left transition ${active ? brandClasses.activeTab : 'border-transparent text-slate-300 hover:border-white/10 hover:bg-white/10 hover:text-white'}`}
              >
                <div className="flex items-start gap-3">
                  <Icon className={`mt-0.5 h-5 w-5 ${active ? brandClasses.accentTextSoft : 'text-slate-500'}`} />
                  <div>
